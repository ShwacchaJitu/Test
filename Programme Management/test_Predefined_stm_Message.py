...
def take_screenshot(org_login, request):
    ...
    screenshots_dir = r"D:\Testcase\screenshots/Predefined_stm_Message"
...
def test_Clinician_Predefined_Messages_page(org_login, take_screenshot):
    driver = org_login
    ...
    driver.find_element(By.XPATH, "//a[@id='Support Team Member (W+)']").click()
    time.sleep(2)
    take_screenshot()
    time.sleep(2)

def test_create_Clinician_Predefined_Messages(org_login, take_screenshot):
    driver = org_login
...
def test_Search_Clinician_Predefined_Messages(org_login, take_screenshot):
    driver = org_login
    driver.find_element(By.XPATH,"//input[@placeholder='Search']").send_keys(message)
    time.sleep(1)
    Predefined_Messages = driver.find_element(By.XPATH,"//tbody/tr[1]/td[2]").text
    assert Predefined_Messages == message
    time.sleep(1)
    take_screenshot()
    time.sleep(2)

def test_Refresh_Clinician_Predefined_Messages(org_login, take_screenshot):
    driver = org_login
    driver.find_element(By.XPATH, "//img[@title='Refresh']").click()
    time.sleep(1)
    take_screenshot()
...
def test_edit_Clinician_Predefined_Messages(org_login, take_screenshot):
    driver = org_login
...
def test_delete_Clinician_Predefined_Messages(org_login, take_screenshot):
    driver = org_login
